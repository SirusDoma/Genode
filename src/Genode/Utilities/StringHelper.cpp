...
namespace Gx
{
    bool StringHelper::IsGlobMatch(const std::string& input, const sf::String& pattern, const bool caseSensitive)
    {
    ...
    bool StringHelper::EqualsCaseInsensitive(const std::string& a, const std::string& b)
    {
    ...
    sf::String StringHelper::TrimStart(const sf::String& input)
    {
    ...
    sf::String StringHelper::TrimEnd(const sf::String& input)
    {
        if (input.getSize() == 1 && std::isspace(static_cast<unsigned char>(input[0])))
            return sf::String();
    ...
    sf::String StringHelper::Trim(const sf::String& input)
    {
        return TrimEnd(TrimStart(input));
    }

    std::string StringHelper::RemoveExtension(const std::string& fileName)
    {
    ...
    std::string StringHelper::GetTypeName(const std::type_info& type, const bool withNamespace)
    {
    ...
    sf::String StringHelper::ToPascalCase(const sf::String& input)
    {
        sf::String result = "";
        bool transform = true;
    ...
    std::vector<std::string> StringHelper::Split(const std::string& input, const char delimiter)
    {
    ...
    sf::String StringHelper::Unquote(const sf::String& string)
    {
    ...
    sf::String StringHelper::ToString(const int value, const int totalLength)
    {
    ...
    sf::String StringHelper::ToString(const float value, const int precision)
    {
        if (precision == 0)
            return std::to_string(value);
